...
app.post("/register", (req, res) => {
  ...
  db.getConnection((err, connection) => {
    ...
    connection.beginTransaction((err) => {
      if (err) {
        res.send("Erro ao iniciar transação");
        return;
      }
      ...
      connection.query("SELECT * FROM usuarios WHERE email = ?", [email], (err, result) => {
        if (err) {
          console.log("Erro ao verificar existência de email:", err);
          res.send("Erro ao verificar email");
          connection.rollback(() => connection.release());
          return;
        }

        if (result.length === 0) {
          // Hash da senha e primeiro INSERT
          bcrypt.hash(password, saltRounds, (err, hash) => {
            if (err) {
              console.log("Erro ao hashear senha:", err);
              res.send("Erro ao hashear senha");
              connection.rollback(() => connection.release());
              return;
            }
            ...
            connection.query(
              "INSERT INTO usuarios (email, password, profissao, nome, phone, cep, logradouro, numero, bairro, cidade, estado, data_nascimento, empresa) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
              [email, hash, profissao, nome, phone, cep, logradouro, numero, bairro, cidade, estado, data_nascimento, empresa],
              (err, response) => {
                if (err) {
                  console.log("Erro ao inserir na tabela usuarios:", err);
                  res.send("Erro ao inserir usuário");
                  connection.rollback(() => connection.release());
                  return;
                }

                // Segundo INSERT na tabela usuario_curso
                connection.query(
                  "INSERT INTO usuario_curso (email_usuario, ID_CURSO) VALUES (?, NULL)",
                  [email],
                  (err, response) => {
                    if (err) {
                      console.log("Erro ao inserir na tabela usuario_curso:", err);
                      res.send("Erro ao inserir curso");
                      connection.rollback(() => connection.release());
                      return;
                    }

                    // Se tudo foi bem-sucedido, fazemos o commit
                    connection.commit((err) => {
                      if (err) {
                        console.log("Erro ao fazer commit:", err);
                        res.send("Erro ao finalizar transação");
                        connection.rollback(() => connection.release());
                        return;
                      }
